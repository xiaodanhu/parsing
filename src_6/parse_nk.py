...
class NKChartParser(nn.Module):
    ...
    def parse_batch(self, sentences, golds=None, return_label_scores_charts=False):
        ...
        # total_loss = tag_loss_action + torch.exp(torch.tensor([self.spec['hparams']['weight_phrase']])).to('cuda') * tag_loss_phrase + torch.exp(torch.tensor([self.spec['hparams']['weight_activity']])).to('cuda') * tag_loss_activity
        total_loss = self.spec['hparams']['weight_action'] * tag_loss_action + self.spec['hparams']['weight_phrase'] * tag_loss_phrase + self.spec['hparams']['weight_activity'] * tag_loss_activity
