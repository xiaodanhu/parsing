...
def decode(force_gold, n_frame, label_scores_chart, is_train, gold, label_vocab, max_branch=20):
    NEG_INF = -np.inf

    label_scores_chart_copy = label_scores_chart.copy()
    value_chart = np.zeros((n_frame-1, n_frame-1), dtype=np.float32)
    split_idx_chart = np.zeros((n_frame-1, n_frame-1), dtype=np.int32)
    best_label_chart = -np.ones((n_frame-1, n_frame-1), dtype=np.int32)
...
################
    ...
    while stack_idx > 0:
        ...
        included_j.append(j)
        #print(i,j)
        included_label.append(best_label_chart[i, j])
        idx += 1
        if i + 1 < j:
