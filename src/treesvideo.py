...
class InternalTreebankNode(TreebankNode):
    ...
    def count_depth(self, depth):
        if self.left is not None:
            self.left.count_depth(depth+1)
        if self.right is not None:
            self.right.count_depth(depth+1)
        self.depth = depth
...
class LeafTreebankNode_alt(TreebankNode):
    ...
    def linearize(self):
        return "({} {})".format(self.tag, self.I3D)
...
# sum([len(train_treebank[i].children) for i in range(len(train_treebank))])/len(train_treebank) # number of phrases per activity.
# sum([np.mean([len(l.children) for l in train_treebank[i].children]) for i in range(len(train_treebank))])/len(train_treebank) # number of actions per phrase.
# sum([np.mean([np.mean([len(m.children) for m in l.children]) for l in train_treebank[i].children]) for i in range(len(train_treebank))])/len(train_treebank) # number of frames per action.
...
def load_trees_alt(mode, path='/hdd/xiaodan8/dataset/FineGym'):
