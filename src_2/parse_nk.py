...
class NKChartParser(nn.Module):
    ...
    def __init__(
        ...
        # print('------------> Vocabulary', frame_vocab.size())
        # to predict phrase labels
        ...
        # layer to predict the action labels
        self.f_tag_action = nn.Sequential(
            nn.Linear(hparams.d_model, hparams.d_tag_hidden),
            LayerNormalization(hparams.d_tag_hidden),
            nn.ReLU(),
    ...
    def parse_batch(self, sentences, golds=None, return_label_scores_charts=False):
        ...
        for snum, sentence in enumerate(sentences):
            for (tag, ph_tag, word) in [(START, START, START)] + sentence + [(STOP, STOP, STOP)]:
                batch_idxs[i] = snum
                # tag_idxs[i]=tag
                i += 1
        ...
        max_word_len = max([max([len(word) for tag,_, word in sentence]) for sentence in sentences])
        ...
        gold_ph_tag_idxs = []
        for snum, sentence in enumerate(sentences):
            for wordnum, (tag, ph_tag, word) in enumerate([(START, START, START)] + sentence + [(STOP, STOP, STOP)]):

                if word in (START, STOP):
                    frame_embeds.append(torch.zeros((2048)))
                    tags.append(tag)
                    ph_tags.append(ph_tag)
                    gold_tag_idxs.append(self.action_vocab.index(tag))
                    gold_ph_tag_idxs.append(self.frame_vocab.index(ph_tag))
                else:
                    ...
                    # print(ph_tag)
                    if len(ph_tag)==1:
                        gold_ph_tag_idxs.append(self.frame_vocab.index(ph_tag[0]))
                    else:
                        gold_ph_tag_idxs.append(self.frame_vocab.index(ph_tag))
                    # gold_ph_tag_idxs.append(self.frame_vocab.index(ph_tag))
        ...
        if PRINT:
            print("Gold ind ", gold_tag_idxs)
            print("Gold ind phrase", gold_ph_tag_idxs)
        gold_tag_idxs = torch.tensor(gold_tag_idxs).cuda()
        gold_ph_tag_idxs = torch.tensor(gold_ph_tag_idxs).cuda()
        frame_embeds = torch.vstack(frame_embeds).cuda()
        # gold_tag_idxs = torch.vstack(gold_tag_idxs).cuda()
        ...
        # if self.f_tag is not None:
        tag_logits = self.f_tag_action(tag_annotations)
        if PRINT: print("------> Tags: ", tag_logits.shape)
        # if is_train:
        tag_loss =  nn.functional.cross_entropy(tag_logits, gold_tag_idxs, reduction='sum')

        if PRINT:print("------> Tags Loss: ", tag_loss)

        ph_tag_logits = self.f_tag(tag_annotations)
        # ph_tag_logits = self.f_label(tag_annotations)

        # print("------> Tags Phrase: ", ph_tag_logits.shape)
        # if is_train:
        ph_tag_loss = nn.functional.cross_entropy(ph_tag_logits, gold_ph_tag_idxs, reduction='sum')

        if PRINT:print("------> Tags Phrase Loss: ", ph_tag_loss)
        ...
        total_loss = loss + 0.001*tag_loss + 0.001*ph_tag_loss
        if PRINT: print("Total Loss: ", total_loss.data)
        # exit()
        return None, (loss, tag_loss, ph_tag_loss)
        # return None, (loss, tag_loss, ph_tag_loss)
            # return charts
