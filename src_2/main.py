...
def run_train(args, hparams):
    ...
    print("===============> ", train_parse[0].label)
    print("===============> ", train_parse[1].label)
    print("===============> ", train_parse[3].label)
    print("===============> ", train_parse[4].label)
    ...
    tag_label=loaders['train'].dataset.id2actionlabel
    ...
    if args.task == 'video':
        def vocab_helper(tree):
            ...
            if type(tree)==treesvideo.InternalParseNode: # and not tree.label[0].strip().isdigit():
                ...
                if len(tree.children)>0: # to keep only the top comment 3 lines
        ...
        # print("--------------> Action Label", action_set, ap_set, apa_set)
        # print("--------------> Phraze Label", id2phraselabel)
        # print("--------------> Action Label", tag_label)


        node_vocab=vocabulary.Vocabulary()
        for i in id2phraselabel:
            # print(tag_label[i])
            node_vocab.index(id2phraselabel[i])
        node_vocab.index('<START>')
        node_vocab.index('<STOP>')
        ...
        for i in tag_label:
            # print(tag_label[i])
            action_vocab.index(tag_label[i])
        ...
        # print("Vocabulary", node_vocab)
        # print("Vocabulary", node_vocab.value(3))
        # exit()
        '''
        node_vocab=vocabulary.Vocabulary()
        node_vocab.index(())
    ...
    def check_dev():
        ...
        for dev_start_index in range(0, len(dev_treebank), args.eval_batch_size):
            ...
            # subbatch_sentences = [[(tree.label, leaf.I3D) for leaf in tree.leaves()] for tree in subbatch_trees]
            # print("Sentence ------->", subbatch_sentences)
        ...
        for train_start_index in range(0, len(train_treebank), args.eval_batch_size):
            ...
            # print("TREEEEEEEEEEEEEEEEEEEE ------->", subbatch_trees[0].label)
            subbatch_sentences = [[(leaf.tag, tree.label, leaf.I3D) for leaf in tree.leaves()] for tree in subbatch_trees]
            # subbatch_sentences = [[(tree.label, leaf.I3D) for leaf in tree.leaves()] for tree in subbatch_trees]
            # print(subbatch_sentences[0])
            predicted, loss = parser.parse_batch(subbatch_sentences)
    ...
    for epoch in itertools.count(start=1):
        ...
        for start_index in range(0, len(train_parse), args.batch_size):
            ...
            batch_sentences = [[(leaf.tag, tree.label, leaf.I3D) for leaf in tree.leaves()] for tree in batch_trees]
            ...
            for subbatch_sentences, subbatch_trees in parser.split_batch(batch_sentences, batch_trees, args.subbatch_max_tokens):
                ...
                loss = loss[0] / len(batch_trees) + 0.001*loss[1] / batch_num_tokens + 0.001*loss[2] / batch_num_tokens
                # loss =  loss[2] / batch_num_tokens
                loss_value = float(loss.data.cpu().numpy())
                # print(loss_value)
