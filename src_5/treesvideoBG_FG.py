...
def load_trees_wBG(mode, path, loaders):
    ...
    for record in loaders[mode].dataset:
        ...
        vid_feature = np.load(os.path.join(feature_folder, 'I3D_' + record[4] + '.npy'))
        vid_feature = torch.from_numpy(np.array([i[0] for i in vid_feature]))
        # if a>40:
        #     break
        ...
        for h in range(len(activities)):
            ...
            for i in range(len(phs)):
                ...
                for j in range(len(ans)):
                    ...
                    if j+1<len(ans):
                        if int(max(record[3][h,i,j])/8)+1<int(record[3][h,i,j+1,0]/8):
                            an_bgs=[]
                            for k in range(int(max(record[3][h,i,j])/8)+1,int(record[3][h,i,j+1,0]/8)):
                                I3D=vid_feature[k]
                ...
                if i+1<len(phs):
                    curr_ed=[int(j/8) for j in record[3][h,i].flatten() if j!=0][-1]
                    next_st=int(record[3][h,i+1,0,0]/8)
            ...
            if h+1<len(activities):
                curr_ed=[int(j/8) for j in record[3][h].flatten() if j!=0][-1]
                next_st=int(record[3][h+1,0,0,0]/8)
                if curr_ed+1<next_st:
                    act_bgs=[]
                    ph_bgs=[]
                    an_bgs=[]
        ...
        trees.append((InternalTreebankNode('tree', ch_vdo), record[4]))
                      
    return trees

def load_trees_wBG0(mode, path):
    ...
    for record in loaders[mode].dataset:
        ...
        ch_tree = []
        #print(a_ed,a_st)
        for l, act in enumerate(record[1]['activity']):
            if act <= 0 or act > 4:
                continue
            ch_act=[]
            phs=[i for i in record[1]['phrase'][l] if i !=-1]
            last_ph=a_st
            for i in range(len(phs)):
                ph=int(phs[i])
                ch_ph=[]
                ans=[j for j in record[1]['action'][l,i] if j !=-1]
                for j in range(len(ans)):
                    an=int(ans[j])
                    ch_an=[]
                    frs=[k for k in record[2][l,i,j] if k!=0]
                    for k in range(len(frs)):
                        I3D=record[0][l,i,j,k]
                        ch_an.append(LeafTreebankNode_alt(id2actionlabel[an], I3D))
                    if(len(ch_an)!=0):
                        ch_ph.append(InternalTreebankNode(id2actionlabel[an], ch_an))

                    if j+1<len(ans):
                        if int(max(record[3][l,i,j])/8)+1<int(record[3][l,i,j+1,0]/8):
                            an_bgs=[]
                            for k in range(int(max(record[3][l,i,j])/8)+1,int(record[3][l,i,j+1,0]/8)):
                                I3D=vid_feature[k]
                                an_bgs.append(LeafTreebankNode_alt('BG_Action', I3D))
                            if(len(an_bgs)!=0):
                                ch_ph.append(InternalTreebankNode('BG_Action', an_bgs))
                ...
                if i+1<len(phs):
                    curr_ed=[int(j/8) for j in record[3][l,i].flatten() if j!=0][-1]
                    next_st=int(record[3][l,i+1,0,0]/8)
                    if curr_ed+1<next_st:
                        ph_bgs=[]
                        an_bgs=[]
                        for k in range(curr_ed+1,next_st):	
                        ...
                        if(len(ph_bgs)!=0):
                            ch_act.append(InternalTreebankNode('BG_Phrase', ph_bgs))
            # if 0<act<=4:
            ch_tree.append(InternalTreebankNode(id2activitylabel[act], ch_act))
            # else:
            #     ch_tree.append(InternalTreebankNode('UNK', ch_act))
        trees.append(InternalTreebankNode('tree', ch_tree))
    return trees
