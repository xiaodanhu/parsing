...
class NKChartParser(nn.Module):
    ...
    def parse_batch(self, sentences, golds=None, return_label_scores_charts=False):
        ...
        if PRINT:
            ...
            print("save results: ")
            predicted = {'pred_action': predictions_action.cpu().numpy(), 'pred_phrase': predictions_phrase.cpu().numpy(), 'pred_activity': predictions_activity.cpu().numpy(), 'gold_action': gold_tag_action_idxs.cpu().numpy(), 'gold_phrase': gold_tag_phrase_idxs.cpu().numpy(), 'gold_activity': gold_tag_activity_idxs.cpu().numpy(), 'vocab_action': self.action_vocab.indices, 'vocab_phrase': self.phrase_vocab.indices, 'vocab_activity': self.activity_vocab.indices}
        ...
        total_loss = tag_loss_action + 1 * tag_loss_phrase + 1 * tag_loss_activity
        
        return (correct_action, correct_phrase, correct_activity), total_loss
