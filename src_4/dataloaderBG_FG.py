...
class Record:
    def __init__(self, vid, locations, masks, fps, interval, window_size, activity_id, phrase_id, action_id, s_len, action_label, phrase_label, activity_label):
        self.vid = vid
...
class CubeDataset(torch.utils.data.Dataset):
    ...
    def load_file(self, root:str, mode:str):
        ...
        anno_label_path = os.path.join(root, "gym99_" + f"{mode}.txt")
        anno_path = os.path.join(root, "valid_anno.json")
        with open(anno_path, 'r') as f:
            data = json.load(f)
            ...
            for line in f:
                ...
                if vname in data.keys() and \
                    ...
                    if flag_save:
                        ...
                        masks.append(mask)
                        records.append(Record(vname_old, sample, mask, self.vid_info[vname_old]['fps'], self.interval, self.max_movment_len_per_action, activity_id_old, phrase_id_old, action_id_old, s_len, action_label, phrase_label, activity_label))
                        sample = np.zeros((self.max_activity_len_per_record, self.max_phrase_len_per_activity, self.max_action_len_per_phrase, self.max_movment_len_per_action))
                        mask = np.zeros((self.max_activity_len_per_record, self.max_phrase_len_per_activity, self.max_action_len_per_phrase, self.max_movment_len_per_action))
                        s_len = np.zeros((self.max_activity_len_per_record, self.max_phrase_len_per_activity, self.max_action_len_per_phrase))
                        action_label = np.zeros((self.max_activity_len_per_record, self.max_phrase_len_per_activity, self.max_action_len_per_phrase)) - 1
    ...
    def get_data(self, record: Record):
        ...
        snippet_fts = torch.zeros((dim0, dim1, dim2, dim3, 2048))
        for l in range(dim0):
            for i in range(dim1):
                for j in range(dim2):
                    for k in range(dim3):
                        if record.masks[l, i, j, k] == 0:
                            break
                        ft_idx = min(torch.floor_divide(og_locations[l, i, j, k], 8).int(), len(vid_feature) - 1)
                        snippet_fts[l, i, j, k, :] = torch.from_numpy(vid_feature[ft_idx].squeeze())
